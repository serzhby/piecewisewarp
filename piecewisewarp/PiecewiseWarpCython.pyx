...
def warpTriangle(imgIn, srcVertices, trgVertices, triangles, imageSize):
	...
	for tr in triangles:
		...
		trianglePoints = np.array([[(p1X, p1Y)], [(p2X, p2Y)], [(p3X, p3Y)]], dtype=np.float32)
		...
		for j in xrange(minBoundY, maxBoundY + 1):
			...
			for i in xrange(minBoundX, maxBoundX + 1):
				if  0 <= lambdaArray1 <= 1 and 0 <= lambdaArray2 <= 1 and 0 <= lambdaArray3 <= 1:
					posuvX.append(lambdaArray1 * s1X + lambdaArray2 * s2X + lambdaArray3 * s3X)
					posuvY.append(lambdaArray1 * s1Y + lambdaArray2 * s2Y + lambdaArray3 * s3Y)
					r.append((j, i))
					length += 1
				lambdaArray1 += g12X
				lambdaArray2 += g20X
